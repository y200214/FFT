...
import numpy as np
from core.constants import EMOTION_VARS, BEHAVIOR_VARS
import os
...
# views/radar_view.py
    ...
    def save_plot(self, output_folder, slope_dfs_data, progress_callback=None, timestamp=0, cancel_check=lambda: False):
        """
        データを受け取り、IDごとに単一のレーダーチャート画像をファイルに保存する。
        """
        print("INFO: レーダーチャートの一括保存を開始します。")
        slope_df = slope_dfs_data.get('full')
